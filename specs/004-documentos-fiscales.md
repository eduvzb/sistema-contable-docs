...
- Frontend: carga múltiple, resultado por archivo, bandeja local con filtros básicos y detalle seleccionable del CFDI, sin añadir descarga SAT ni automatizaciones fiscales. El detalle presenta los campos del contrato existente, sin recalcular importes ni convertir moneda.
...
**Evidencia de continuidad 2026-09-08:** el detalle de la bandeja consulta `GET /api/companies/{companyId}/fiscal-documents/{fiscalDocumentId}` y presenta los campos fiscales del contrato, sin recalcular importes. La prueba `Spec004Test::test_document_detail_exposes_the_complete_fiscal_contract` verifica el contrato completo; la prueba focalizada pasó con 4 pruebas y 41 aserciones y la suite backend completa pasó con 36 pruebas y 321 aserciones. En frontend, `pnpm lint`, `pnpm exec tsc --noEmit` y `pnpm run build` pasaron. No se ejecutó navegador ni Playwright.

**Continuidad de contexto 2026-09-08:** la bandeja ya no emite consultas sin `period_id`: sólo se monta después de validar explícitamente el periodo contra la empresa y limpia documentos/detalle al perderlo. `pnpm lint`, `pnpm build` y `pnpm typecheck` pasan; la comprobación visual continúa pendiente.
...
- **2026-09-08:** se concretó la consulta de detalle del CFDI en la bandeja mediante el contrato `GET` existente y se añadió cobertura automatizada de sus campos fiscales. Permanece pendiente la comprobación interactiva por la restricción de no usar navegador.
